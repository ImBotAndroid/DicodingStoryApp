...
import com.example.submissiondicodingintermediate_1.ui.utils.MainDispatcherRule
...
class MainViewModelTest{
    ...
    fun `when Get Story Should Not Null and Return Success`() = runTest {
        val bearer = "Token"
        val dummyStory = DataDummy.dummyListStory()
        val data: PagingData<ListStoryItem> = StoryPagingSource.snapshot(dummyStory)
        val expectedStory = MutableLiveData<PagingData<ListStoryItem>>()

        expectedStory.value = data
        Mockito.`when`(repository.getAllStory(bearer)).thenReturn(expectedStory)

        val mainViewModel = MainViewModel(repository)
        val actualStory: PagingData<ListStoryItem> = mainViewModel.getAllStory(bearer).getOrAwaitValue()
